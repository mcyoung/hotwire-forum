...
class Discussion < ApplicationRecord
  ...
  def subscribed_reason(user)
    return "You're not receiving notifications from this thread 1" if user.nil?

    if (subscription = subscription_for(user))
      if subscription.subscription_type == "optout"
        "You're ignoring this thread"
      elsif subscription.subscription_type == "optin"
        "You're receiving notifications because you've subscribed"
      end
    elsif posts.where(user_id: user.id).any?
        "You've posted and therefore you're receiving notifications"
    else
      "You're not receiving notifications from this thread 2"
    end
  end
end
